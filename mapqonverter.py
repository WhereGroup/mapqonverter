...
def main():
    ...
    for counter, dataframe in enumerate(arcpy.mapping.ListDataFrames(mxd)):
        if counter == 0:
            ...
            MapSpatialReferenceSystem.create_map_srs_element(dom, header, dataframe)
        ...
        ProjectLayers.create_project_layers_element(dom, header, layer_list, arc_dataframe)
        ...
        MapLegend.create_map_legend_element(dom, header, layer_list, dataframe)
        LayerTree.create_layertree(dom, header, layer_list, dataframe)
        MapProperties.create_map_properties_element(dom, header)

    try:
        dom.writexml(qgs_file, indent='  ', addindent='  ', newl='\n', encoding='UTF-8')
